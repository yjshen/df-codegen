...
use crate::{estimate_row_width, fixed_size, fn_name, get_offsets, reg_fn, supported};
use arrow::array::*;
...
pub fn write_batch_unchecked_jit(
    ...
    let code_fn =
        unsafe { std::mem::transmute::<_, fn(&mut RowWriter, usize, &RecordBatch)>(code_ptr) };
...
pub fn bench_write_batch_jit_dummy(
    schema: Arc<Schema>,
) -> Result<*const u8> {
    let assembler = Assembler::default();
    register_write_functions(&assembler)?;
    let gen_func = gen_write_row(&schema, &assembler)?;
    let mut jit = assembler.create_jit();
    let code_ptr: *const u8 = jit.compile(gen_func)?;
    Ok(code_ptr)
}
...
fn gen_write_row(schema: &Arc<Schema>, assembler: &Assembler) -> Result<GeneratedFunction> {
    use DataType::*;
    ...
    for (i, f) in schema.fields().iter().enumerate() {
        ...
        if f.is_nullable() {
            b.if_block(
                ...
                |e| {
                    e.call_stmt("set_non_null_at", vec![e.id("row")?, e.lit_i(i as i64)])?;
                    let params = vec![
                        e.id("row")?,
                        e.id(&arr)?,
                        e.lit_i(i as i64),
...
fn write_field_date32(to: &mut RowWriter, from: &Arc<dyn Array>, col_idx: usize, row_idx: usize) {
    let from = from.as_any().downcast_ref::<Date32Array>().unwrap();
    to.set_date32(col_idx, from.value(row_idx));
}

fn write_field_date64(to: &mut RowWriter, from: &Arc<dyn Array>, col_idx: usize, row_idx: usize) {
    let from = from.as_any().downcast_ref::<Date64Array>().unwrap();
    to.set_date64(col_idx, from.value(row_idx));
}

fn write_field_utf8(to: &mut RowWriter, from: &Arc<dyn Array>, col_idx: usize, row_idx: usize) {
...
fn write_field_binary(to: &mut RowWriter, from: &Arc<dyn Array>, col_idx: usize, row_idx: usize) {
